...
// Check the email address enmtered matches the standard email address format
 if (!eregi("^[A-Z0-9._%-]+@[A-Z0-9._%-]+\.[A-Z]{2,6}$", $email)) { 
  echo "<p>Vous avez entré une mauvaise adresse email</p><p><a href='javascript: history.go(-1)'>Cliquer ici pour retourner à la page précédente</a>.</p>"; 
} 

 elseif (!trim($name)) { 
  echo "<p>Entrer votre nom</p><p><a href='javascript: history.go(-1)'>Cliquer ici pour retourner à la page précédente</a>.</p>"; 
} 


 elseif (!trim($message)) { 
  echo "<p>Entrer votre message</p><p><a href='javascript: history.go(-1)'>Cliquer ici pour retourner à la page précédente</a>.</p>"; 
}  

 elseif (!trim($email)) { 
  echo "<p>Entrer un email</p><p><a href='javascript: history.go(-1)'>Cliquer ici pour retourner à la page précédente</a>.</p>"; 
} 

// Sends out the email or will output the error message 
 elseif (mail($sendto, $subject, $emailcontent, $headers)) { 
  echo "<br><br><p><b>Merci $name</b></p><p>On vous contactera dés que possible.</p>"; 

} 
} 
else { 
?> 
<p align="center" class="corsiva1">Contacter Nous...</p>
<br>
<form method="post"><INPUT NAME="op" TYPE="hidden" VALUE="send"> 
  <table> 
    <tr> 
      <td><p class="corsiva4">Nom:</p></td> 
      <td> 
    ...
    <tr><td></td> <td><input name="submit" type="submit" value="Envoyer Message"></td></tr> 
  </table> 
</form> 
<?php } ?>
